...
@configure(
    label='[ripVT] - Report to Detected Communicating Samples',
    description='Parses report for communicating samples.',
    uuids=[ 'ripVT.v2.ip_2_rep_comm_samples'],
...
def dotransform(request, response):
    ...
    try:
        items=ast.literal_eval(request.fields['detected_communicating_samples'])
    except:
        return response
